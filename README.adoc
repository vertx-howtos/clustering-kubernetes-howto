...
On Kubernetes, Infinispan shouldn't use the `tcp` transport stack because most often IP multicast is not available.

Instead, it provides a `kubernetes` stack that relies on https://kubernetes.io/docs/concepts/services-networking/service/#headless-services[headless service] lookup for discovery and TCP connections for group messaging:

ifdef::env-github[]
link:backend/src/main/resources/infinispan-kubernetes.xml[Infinispan Kubernetes]
endif::env-github[]
ifndef::env-github[]
[source,java,role="collapsed"]
.XML `backend/src/main/resources/infinispan-kubernetes.xml`
----
include::backend/src/main/resources/infinispan-kubernetes.xml[]
----
endif::env-github[]
...
* `vertx.infinispan.config`: alternative Infinispan configuration file with `kubernetes` transport stack
...
We also need a service to expose our pods to the outside world and load-balance the HTTP traffic.
...
First, let's ask Minikube to give the url of the `frontend` service from the development machine:
...
----
http://172.17.0.3:30729
----

If you don't use Minikube, you can get the external IP with `kubectl get services frontend`.

Then we can send a request to the _frontend_ for the `/hello` URI with the `name` query param set to `Vert.x Clustering`

----
http 172.17.0.3:30729/hello name=="Vert.x Clustering"
----
